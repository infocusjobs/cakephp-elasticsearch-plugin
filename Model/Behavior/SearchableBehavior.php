...
class SearchableBehavior extends ModelBehavior
{
    ...
    protected $_default = array(
        ...
        'enforce' => array(),
        'fields' => '_all',
        'fields_excludes' => array('_url', '_model', '_label')
    );
    ...
    {
        if (
            substr($className, 0, strlen(self::$_autoLoaderPrefix)) !==
            self::$_autoLoaderPrefix
        ) {
    ...
    public static function allModels($instantiated = false)
    {
        ...
        foreach (CakePlugin::loaded() as $plugin) {
            foreach (
                glob(CakePlugin::path($plugin) . 'Model' . DS . '*.php')
                as $filePath
            ) {
    ...
    protected static function _getModel(
        ...
        if (
            !$Model->Behaviors->attached('Searchable') ||
            !$Model->elastic_enabled()
        ) {
            return false;
        }
    ...
    public function fill()
    {
        ...
        while (true) {
            ...
            if ($curCount < $limit) {
                $this->progress(
                    $Model,
                    'Reached curCount ' . $curCount . ' < ' . $limit
                );
                break;
            }
            $offset += $limit;
    ...
    protected function _fillChunk($Model, $offset, $limit, $id = null)
    {
        ...
        if ($isQuery) {
            ...
            $this->progress(
                $Model,
                '(select_start: ' . $offset . '-' . ($offset + $limit) . ')'
            );
        ...
        foreach ($results as $result) {
            ...
            if (!@$Model->title) {
                $Model->title = Inflector::humanize(
                    Inflector::underscore($doc['_model'])
                );
            }
    ...
    public function execute(
        ...
        if (!array_key_exists('fullIndex', $options)) {
            $options['fullIndex'] = $Model->fullIndex;
        }
        ...
        curl_setopt(
            ...
            $this->opt($Model, 'curl_connect_timeout')
        );
        curl_setopt(
            $conn,
            CURLOPT_TIMEOUT,
        ...
        if (false === $response) {
            return sprintf(
                'Invalid response from elasticsearch server while contacting %s (%s)',
                $uri,
                $json
            );
        }
        if (@$response['error']) {
            if (
                $response['error'] ===
                'ActionRequestValidationException[Validation Failed: 1: no requests added;]'
            ) {
                return $response;
            }
            return sprintf(
    ...
    {
        $key = join(',', array($Model->name, $Model->fullIndex));
    ...
    protected function _filter_highlight($Model, $val)
    {
        ...
        if ($params = @$val['fields']['_all']) {
            ...
            if (
                false !== ($k = array_search('_no_all', $val['fields'], true))
            ) {
                return $val;
            }
    ...
    public function err(
        ...
        if (@$this->settings[$Model->alias]['error_handler'] === 'php') {
            ...
            @$this->settings[$Model->alias]['error_handler'] === 'disklog'
        ) {
            CakeLog::error($str);
        }
