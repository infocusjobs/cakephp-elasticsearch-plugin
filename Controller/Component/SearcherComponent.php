...
class SearcherComponent extends Component
{
    ...
    public function searchAction($ajax)
    {
        if ($this->opt('model') === '_all') {
            $this->LeadingModel = ClassRegistry::init(
                $this->opt('leading_model')
            );
        ...
        if (
            ...
            $this->mOpt($this->LeadingModel, 'searcher_action')
        ) {
            return null;
        }

        if (
            !($query = @$this->Controller->passedArgs[
                $this->mOpt($this->LeadingModel, 'searcher_param')
            ])
        ) {
            if (
                !($query = @$this->Controller->data[
                    $this->mOpt($this->LeadingModel, 'searcher_param')
                ])
            ) {
    ...
    {
        $raw_results = $this->LeadingModel->elastic_search(
            $query,
            $queryParams
        );
    ...
    public function respond($response)
    {
        ...
        if (
            @$GLOBALS['XHPROF_ON'] &&
                @$GLOBALS['XHPROF_NAMESPACE'] && @$GLOBALS['TIME_START']
        ) {
            $xhprof_data = xhprof_disable();
            $xhprof_runs = new XHProfRuns_Default();
            $run_id = $xhprof_runs->save_run(
                $xhprof_data,
                $GLOBALS['XHPROF_NAMESPACE']
            );
            $parsetime = number_format(
                getmicrotime() - $GLOBALS['TIME_START'],
                3
            );
            $xhprof = sprintf(
    ...
    public function isEnabled($Controller)
    {
        ...
        return $Model->Behaviors->attached('Searchable') &&
